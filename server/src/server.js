...
const Settings = require('./services/settings/settings.model');

const userRoutes = require('./services/user/user.routes');
const settingsRoutes = require('./services/settings/settings.routes');
const authRoutes = require('./services/auth/auth.routes');
const videoRoutes = require('./services/video/video.routes');
const auditRoutes = require('./services/audit/audit.routes');
const { createAuditMiddleware } = require('./services/audit/audit.middleware');
...
const morganFormat = config.server.nodeEnv === 'development' 
  ...
  try {
    ...
    for (const [category, keys] of Object.entries(criticalSettings)) {
      ...
      if (missingSettings.length > 0) {
        console.log(`Missing critical settings in ${category}: ${missingSettings.join(', ')}`);
        console.log('Initializing missing settings...');
        await Settings.initializeDefaults();
        return;
      }
